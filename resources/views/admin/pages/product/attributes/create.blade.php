...
@section('content')
    <div class="row mb-4">
        <div class="col d-flex justify-content-between align-items-center">
            ...
            <a class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#modal-report">Thêm thuộc tính
                {{ $atrributeName }}
            </a>
        </div>
    </div>
                    ...
                    <tr data-id="{{ $id }}" data-routename="{{ $routeName }}">
                        ...
                        <td>
                            <button class="btn btn-outline-danger item_delete">
                                ...
                                <form action="{{ route('admin.' . 'attribute-values' . '.destroy', ['item' => $item]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                </form>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Thêm thuộc tính {{ $atrributeName }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                {{-- lưu vào table atrribute_values --}}
                <form action="{{ route('admin.' . 'attribute-values' . '.store') }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="hidden" name="attribute_id" value="{{ $atrributeId }}">
                    </div>
                </form>
            </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const nameInput = document.querySelector('.modal input[name="value"]');
