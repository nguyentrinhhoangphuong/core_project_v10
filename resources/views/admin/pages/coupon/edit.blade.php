...
@section('content')
    ...
    <form action="{{ route('admin.coupon.update', ['item' => $item['id']]) }}" method="post" class="card">
        @csrf
        @method('PUT')
        <div class="card-body">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="discount_type" required>
                                    <option value="fixed"
                                        {{ old('discount_type', $item['discount_type']) == 'fixed' ? 'selected' : '' }}>
                                        Cố định
                                    </option>
                                    <option value="percent"
                                        {{ old('discount_type', $item['discount_type']) == 'percent' ? 'selected' : '' }}>
                                        Phần trăm
                                    </option>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control" name="discount_value"
                                    value="{{ old('discount_value', $item['discount_value']) }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" class="form-control" name="starts_at"
                                    value="{{ old('starts_at', $item['starts_at']) }}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Ngày kết thúc</label>
                                <input type="datetime-local" class="form-control" name="expires_at"
                                    value="{{ old('expires_at', $item['expires_at']) }}" required>
                            </div>
